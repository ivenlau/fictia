...
import { Loader2, Save, FolderTree, FileText } from "lucide-react";
import { useUpdateNovel } from "@/hooks/useNovel";
import { FILE_TEMPLATES, AGENT_TYPE_LABELS } from "@fictia/shared";
import type { Novel, AgentType } from "@fictia/shared";
...
export function EditNovelModal({ novel, onClose }: EditNovelModalProps) {
  const updateNovel = useUpdateNovel(novel.id);

  const existingTags: string[] = Array.isArray(novel.tags)
    ? novel.tags
    : typeof novel.tags === "string"
      ? (() => { try { return JSON.parse(novel.tags); } catch { return []; } })()
      : [];

  const [title, setTitle] = useState(novel.title);
  const [genre, setGenre] = useState(novel.genre);
  const [description, setDescription] = useState(novel.description ?? "");
  const [targetChapters, setTargetChapters] = useState(novel.targetChapters);
  ...
  }, [title, genre, description, targetChapters, tags, updateNovel, onClose]);
                ...
                {tags.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {tags.map((tag) => (
                      ...
                      >
                        {tag}
                        <button onClick={() => handleRemoveTag(tag)} className="hover:text-accent-deep">&times;</button>
                      </span>
                    ))}
                  </div>
                )}
